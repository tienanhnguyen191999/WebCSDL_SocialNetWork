...
@WebServlet("/view/newsfeed-videos")
public class HandleRenderNewsFeedVideo extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			qr = "select * from post where user_id in(" + String.join(",",arr) + ") and type= 3 order by post_id desc";
			st = new Dao().con.prepareStatement(qr);
			rs = st.executeQuery();
			req.setAttribute("newsfeeds", rs);
			req.getRequestDispatcher("newsfeed-videos.jsp").forward(req, resp);
		} catch (Exception e) {
			e.printStackTrace();
		}
