...
@WebServlet("/view/newsfeed-images")
public class handleRenderNewsFeedImages extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			qr = "select * from post where user_id in(" + String.join(",",arr) + ") and type= 2 order by post_id desc";
			st = new Dao().con.prepareStatement(qr);
			rs = st.executeQuery();
			req.setAttribute("newsfeeds", rs);
			req.getRequestDispatcher("newsfeed-images.jsp").forward(req, resp);
			//resp.sendRedirect("newsfeed-images.jsp");
		} catch (Exception e) {
			e.printStackTrace();
		}
